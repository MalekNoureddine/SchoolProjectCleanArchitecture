...
namespace SchoolProjectCleanArchitecture.Api.Controllers
{
    ...
    public class InstructorsController : AppBaseController
    {
        ...
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        public async Task<ActionResult<Department>> GetPaginatedStudents([FromQuery] GetPaginatedInstructorsListQuery query)
        {
        ...
        public async Task<ActionResult<Department>> GetDepartmentById([FromQuery] GetInstructorByIdQuery query)
        {
            var response = await _mediator.Send(query);
            return NewResult(response);
        }
